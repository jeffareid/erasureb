//----------------------------------------------------------------------//
//      ersbchc.cpp erasure demo - BCH based RS code                    //
...
extern "C++" void MatrixXora(MATRIX &, int);
...
int i, j;
    ...
    InitGFa(abMpy);                     // init assembly table
}
...
int r, c, n, m;
    ...
    for(r = 0; r < NROW; r++){              // single erasures
        ...
        MatrixXora(mDat, r);                // xor non-erased rows
    }
    ...
    for(n = 2; n <= NPAR; n++){             // n = number of erasures
        ...
        while(NextCombination(e, n, NROW)){ // set e == erasure indexes
            ...
            for(r = 0; r < m; r++)          // zero out n erased columns
                for(c = 0; c < n; c++)
                    mCor.m[r][e[c]] = 0;
            MatrixMpya(mFix, mCor, mDat);   // correct m rows
            MatrixXora(mDat, e[m]);         // correct last row via xor
...
BYTE b;
    ...
    MatrixMpya(mPar, mEnc, mDat);           // encode all but last row
    MatrixXora(mDat, NROW-1);               // xor last row ("fix" it)
